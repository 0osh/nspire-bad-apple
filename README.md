...
basic program that plays bad apple using ndless. it uses simple run length encoding (rle) to compress the video file. The final size of the whole program is about 7.7MiB
...
```
change `file_i+=1` to `file_i+=2` to skip every other frame and the file size will half. It doesn't reduce quality too much :)
